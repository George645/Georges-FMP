    ...
    public class KnightScript : MonoBehaviour {
        ...
        void ShowMovablePositions() {
            if (selected && displayedMovementList.Count == 0) {
                ...
                for (int x = -1; x <= 1; x += 2) {
                    for (int z = -1; z <= 1; z += 2) {
                        if (TC.CubeBase.GetSquareInDirection(transform.position.x + 2 * x, transform.position.z + 1 * z)) {
                            displayedMovementList.Add(TC.MoveableDisplays.Instance.GetObject());
                        ...
                        if (TC.CubeBase.GetSquareInDirection(transform.position.x + 1 * x, transform.position.z + 2 * z)) {
                            displayedMovementList.Add(TC.MoveableDisplays.Instance.GetObject());
                            displayedMovementList[count].SetActive(true);
